...
# Извлечение текста из документов и индексация
documents = []
es = Elasticsearch([{'scheme': 'http', 'host': 'localhost', 'port': 9200}])

def index_document_text(file_path, text):
    doc = {
        'file_path': file_path,
        'text': text
    }
    es.index(index='documents', id=file_path, body=doc)

for file_path in rawDocuments:
    text = extract_text_from_document(file_path)
    index_document_text(file_path, text)
